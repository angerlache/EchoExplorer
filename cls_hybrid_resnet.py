...
class NeuralNet:
    ...
    def __init__(self, params_):
        """
        Creates a resnet for features computation and an XGBoost model for detection and classification.
    ...
    def train(self, positions, class_labels, files, durations):
        ...
        if self.params.tune_resnet:
            print("Tune features resnet")
            tic_resnet_8 = time.time()
            tune_network(self.params, features, labels, labels_not_merged, self.params.trials_filename_1)
            toc_resnet_8 = time.time()
            while toc_resnet_8-tic_resnet_8 < self.params.tune_time:
                tune_network(self.params, features, labels, labels_not_merged, self.params.trials_filename_1)
                toc_resnet_8 = time.time()
            print('total tuning time', round(toc_resnet_8-tic_resnet_8, 3), '(secs) =', round((toc_resnet_8-tic_resnet_8)/60,2), r"min \\")
        ...
        self.model_feat = Model(inputs=self.network_features.input,
                                outputs=self.network_features.layers[len(self.network_features.layers)-2].output)
