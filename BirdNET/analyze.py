...
def saveResultFile(r: Dict[str, list], path: str, afile_path: str):
    ...
    #with open("classification_result", "w", encoding="utf-8") as rfile:
...
def combineResults(folder: str, output_file: str):
    ...
    files = utils.collect_all_files(folder, "csv", pattern="BirdNET.results")
    print(files)
    print(folder)
    ...
    s_id = 1
    time_offset = 0
    #print("path2 = ", str(os.path.join(folder, output_file)))
    with open(os.path.join(folder, output_file), "w", encoding="utf-8") as f:
        ...
        f.write("File name,Begin Time (s),End Time (s),Species Code,Confidence\n")
