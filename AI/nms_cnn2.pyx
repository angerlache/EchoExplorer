...
def nms_1d(np.ndarray src, np.ndarray pred_classes, np.ndarray call_predictions_not_bat, np.ndarray features, int win_size, float file_duration):
    ...
    cdef np.ndarray pos = np.empty(width, dtype=np.int32)
    cdef int pos_cnt = 0
    while ii <= width:
